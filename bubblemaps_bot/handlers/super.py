...
from bubblemaps_bot.utils.bubblemaps_api import fetch_distribution, fetch_address_details
from bubblemaps_bot.utils.bubblemaps_metadata import fetch_metadata_from_all_chains
from bubblemaps_bot.utils.screenshot import capture_bubblemap, build_iframe_url
import logging
...
async def check_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not result:
        await update.message.reply_text("❌ No data found for this token on supported chains.")
        return
...
async def send_main_menu(message, context: ContextTypes.DEFAULT_TYPE, edit_message_id=None):
...
async def send_mapshot(message, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await message.reply_text(f"❌ Mapshot failed: {e}")
...
async def send_distribution_page(message, context: ContextTypes.DEFAULT_TYPE, edit_message_id=None):
    ...
    if not data.get("distribution"):
        ...
        if not sorted_nodes:
            await message.reply_text(f"❌ No distribution data found.")
            return
        data["distribution"] = {
            "nodes": sorted_nodes,
            "page": 0
    ...
    for node in current_nodes:
        ...
        keyboard.append([
            InlineKeyboardButton(f"🔍 {address[:6]}...", callback_data=f"check_addr_inline_{address}"),
            InlineKeyboardButton(f"📩 {address[-4:]}", callback_data=f"check_addr_new_{address}")
        ])

    nav_buttons = []
    if page > 0:
        nav_buttons.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"check_dist_prev_{page}"))
    if end_idx < total_items:
        nav_buttons.append(InlineKeyboardButton("Next ➡️", callback_data=f"check_dist_next_{page}"))
...
async def send_address_details_inline(message, context: ContextTypes.DEFAULT_TYPE, address: str, edit_message_id=None):
...
async def send_address_details_new(message, context: ContextTypes.DEFAULT_TYPE, address: str):
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_close":
        ...
        await send_distribution_page(message, context, edit_message_id=check_data["message_id"])
        return

    elif data.startswith("check_dist_prev_") or data.startswith("check_dist_next_"):
        page = int(data.split("_")[-1])
        check_data["distribution"]["page"] = page - 1 if data.startswith("check_dist_prev_") else page + 1
        await send_distribution_page(message, context, edit_message_id=check_data["message_id"])
        return
        ...
        await send_address_details_inline(message, context, address, edit_message_id=check_data["message_id"])
        return
        ...
        if check_data["state"] == "address_inline":
            ...
            await send_main_menu(message, context, edit_message_id=check_data["message_id"])
        return
